...
- A new project with 2 pipelines will be created in MLDM
  - Data (files) will be uploaded to the MLDM repository 
  - The 'Train' pipeline will wait for new images to be committed to the input repository, which will initiate a new Experiment in MLDE
  - To generate a new Experiment, the pipeline will need to download the assets (configuration + code) from github
    - Technically speaking, it can download these assets from anywhere, but github is the easiest way to maintain the code
...
- Each pipeline will pull a specific container image from a registry. There are instructions here on how you can create your own images and push them to your own registry.
...
In this guide, we will deploy one of the example projects (Dogs and Cats), to ensure that all PDK components are working properly. 
...
aws ecr create-repository --repository-name=${NAME}/denisd_cats_dogs_train --region ${AZ_REGION}
##
