...
def xview_to_coco(
    ...
    train_json_path = Path(output_dir) / "train_rgb.json"
    val_json_path = Path(output_dir) / "val_rgb.json"
...
def get_labels(fname,train_img_dir):
    ...
    names = [str(i.name) for i in list(Path(train_img_dir).glob('*.png'))]
    ...
    for i in tqdm(range(len(data["features"])), "Parsing xView data"):
        if data["features"][i]["properties"]["bounds_imcoords"] != []:
            ...
            # print(b_id)
            b_id = str(Path(b_id).with_suffix('.png'))
            # print(b_id)
            # if b_id in names:
            #     print(b_id, b_id in names)
...
if __name__ == "__main__":
    xview_to_coco(
    train_images_dir='/Users/mendeza/data/xview/train_images_rgb/',
    train_geojson_path='/Users/mendeza/data/xview/xView_train.geojson',
    output_dir='/Users/mendeza/data/xview/',
    train_split_rate=0.75,
    category_id_remapping ='/Users/mendeza/Documents/projects/xview-torchvision-coco/utils/category_id_mapping.json',
    xview_class_labels='/Users/mendeza/Documents/projects/xview-torchvision-coco/utils/xview_class_labels.txt')