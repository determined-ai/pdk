...
class RandomHorizontalFlip(T.RandomHorizontalFlip):
    def forward(self, image: Tensor,
        ...
        return image, target
# (Andrew 3.1.2023): This requires torch. 0.12.1
# class RandomHorizontalFlip(T.RandomHorizontalFlip):
#     def forward(
...
class PILToTensor(nn.Module):
    def forward(
        ...
        # nppic = np.zeros([3, image.height, image.width], dtype=np.uint8)
        # image.copyto(nppic)
        # nppic = np.ascontiguousarray(nppic, dtype=np.uint8)
        # iamge  = torch.as_tensor(nppic)
        return image, target
